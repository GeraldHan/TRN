...
class Encoder(nn.Module):
    def __init__(self, input_size, state_size, num_layers):
        ...
        self.lin2 = FCNet(input_size, state_size, drop=0.2)
...
class Decoder(nn.Module):
    def __init__(self, state_size, obs_size, num_layers):
        ...
        self.lin_o = FCNet(state_size, obs_size, drop=0.3)
    def forward(self, input):
        # x = self.lin(input)
        # for layer in self.layers:
            # x = layer(x) + x
...
class BiGRUEncoder_2(nn.Module):
    ...
    def forward(self, input_embedded, input_seq_lens, e_s, e_e):
        ...
        for k in range(max_len - 1):
            hidden_entiy.append(
                torch.cat([forward_outputs[e_e[:,k], torch.arange(batch_size)], backward_outputs[e_s[:,k]-1, torch.arange(batch_size)]], dim=1)
                )
            hidden_trans.append(
